...
const ChatMessage: FC<ChatMessageProps> = ({ children, time, typing = false, isMe }: ChatMessageProps) => {
  return (
    <div className={cn("flex justify-end px-3 w-full mt-2 fade-in-show", isMe ? "justify-end " : "justify-start")}>
      <div className="flex flex-col">
        ...
        {time && !typing && <span className="mt-1 text-[0.5rem] text-gray-400">{getMessageDiffrenceTime(time)}</span>}
      </div>
    </div>
  );
};
